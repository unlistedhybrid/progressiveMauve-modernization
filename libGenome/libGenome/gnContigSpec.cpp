...
void gnContigSpec::CropStart(gnSeqI cropLen) {
    m_start = m_reverseComplement
        ? (GetSourceLength() + m_start - cropLen) % GetSourceLength()
        : (m_start + cropLen) % GetSourceLength();
    m_length -= cropLen;
}
...
) const
{
    ...
    if (contigI == ALL_CONTIGS) {
        read_start = m_reverseComplement
            ? (m_start - start - readable + GetSourceLength()) % GetSourceLength()
            : start + m_start;
    }
    ...
    if (m_circular) {
        gnSeqI beginread = bufLen - readable; // Amount left to read
        read_start = m_reverseComplement
            ? (m_start - readable + GetSourceLength()) % GetSourceLength()
            : m_start;
...
void gnContigSpec::Clear() noexcept {
    gnBaseSpec::Clear();
    m_start = 0;
    m_length = 0;
