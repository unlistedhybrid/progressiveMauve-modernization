...
public:
    gnBaseSpec() = default;
    virtual ~gnBaseSpec() override = default;

    // Copying and assignment are allowed by default
     ...
     */
    virtual bool IsReverseComplement() const;
     ...
     */
    virtual void SetReverseComplement(const bool value) = 0;
     ...
     */
    virtual bool SeqRead(const gnSeqI start, gnSeqC* buf, gnSeqI& bufLen, const uint32 contigI) const = 0;
...
protected:
    bool m_reverseComplement{false};
...
inline std::string gnBaseSpec::GetName() const {
    return m_name;
}
inline void gnBaseSpec::SetName(const std::string& name) {
    m_name = name;
}
inline bool gnBaseSpec::IsReverseComplement() const {
    return m_reverseComplement;
}
inline bool gnBaseSpec::IsCircular() const {
    return m_circular;
}
inline void gnBaseSpec::SetCircular(const bool value) {
    m_circular = value;
}
inline void gnBaseSpec::Clear() {
    m_sourceName.clear();
    m_name.clear();
