...
void print_usage(char* pname){
    cout << "Usage: " << pname << " <genbank file> <exon_list> <intron_list> "
         << "<mem_list> <regulation_network> <minimum match size>\n";
}
...
void map_coordinates(vector<gnLocation>& loc_list, vector<gnLocation>& map_list){
    ...
    for(uint32 locationI = 0; locationI < loc_list.size(); locationI++){
        gnSeqI cur_len = loc_list[locationI].GetEnd() -
                         loc_list[locationI].GetStart() + 1;
...
int main(int argc, char* argv[]){
    ...
    uint32 notify_interval =
        (notify_percent > 0 && mem_list.size() > notify_percent)
        ? mem_list.size() / notify_percent
        : 1;
    ...
    for(uint32 memI = 0; memI < mem_list.size(); memI++){
        ...
        gnLocation ex_map_loc(mem_list[memI].ex_start,
        ...
        gnLocation in_map_loc(cur_in_start,
                              cur_in_start + mem_list[memI].length - 1);

        uint32 search_mapI = loc_binary_search(intron_map_list,
                                               0,
                                               intron_map_list.size()-1,
                                               in_map_loc);
