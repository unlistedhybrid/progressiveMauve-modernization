			...
			for( size_t dI = 0; dI < disjoint_subsets.size(); ++dI )
			{
				...
				for( size_t seqI = 0; seqI < seq_count; ++seqI )
				{
					...
					for( int mI = 0; mI < cur_d_matches.size(); ++mI )
					{
						...
						if( prev != -1 )
						{
							Pair edge( static_cast<int>(id_map[prev]), static_cast<int>(id_map[mI]) );
							if( reverse )
					...
					if( prev != -1 && !reverse )
						edges.push_back( Pair( static_cast<int>(id_map[prev]), static_cast<int>(cur_d_matches.size()) ) );
					else if( prev != -1 && reverse )
						edges.push_back( Pair( static_cast<int>(id_map[first]), static_cast<int>(cur_d_matches.size()) ) );
				}
