...
{
  LPVOID msg = NULL; // Initialize to NULL for safety
  ...
  if (dwRet > 0 && msg != NULL) {
    // Cast msg to (char *) to satisfy the %s format specifier
    printf("%s: %s\n", str, (char *)msg); 
    LocalFree(msg); // Free the buffer allocated by FormatMessage
  } else {
    printf("%s: Unknown error (FormatMessage failed)\n", str);
  }
}
