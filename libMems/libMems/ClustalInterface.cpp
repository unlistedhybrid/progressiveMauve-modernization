...
// If ClustalW C API headers are available:
extern "C" {
...
void ClustalInterface::SetDistanceMatrix(NumericMatrix<double>& distance_matrix, string& tree_filename, bool reread_tree) {
	...
	if (!reread_tree) {
		...
		for (seqI = 0; seqI < dist_plus_matrix.cols(); ++seqI) {
			for (seqJ = 0; seqJ < dist_plus_matrix.cols(); ++seqJ) {
				...
				if (seqI == 0) {
					if (seqJ == 0) new_val = 0;
					else new_val = distance_matrix(seqI, seqJ - 1);
				} else {
					if (seqJ == 0) new_val = distance_matrix(seqI - 1, seqJ);
					else new_val = distance_matrix(seqI - 1, seqJ - 1);
				}
...
bool ClustalInterface::Align(GappedAlignment& cr, Match* r_begin, Match* r_end, vector<gnSequence*>& seq_table) {
	bool flank = false;
	gnSeqI gap_size = 0;
	bool create_ok = true;
...
bool ClustalInterface::CallClustal(vector<string>& seq_table) {
    // Free any pre-existing ClustalW alignment data structures
    free_aln(nseqs);

    // Reallocate for the number of sequences provided
    ...
    // If a distance matrix is available and matches sequence count, use it.
    // Otherwise, create a temporary guide tree file name.
    if (distance_matrix.cols() == seq_table.size()) {
        for (uint seqI = 0; seqI < nseqs; ++seqI) {
            for (uint seqJ = 0; seqJ < nseqs; ++seqJ) {
                tmat[seqI + 1][seqJ + 1] = 1.0 - distance_matrix(seqI, seqJ);
            }
        }
    } else {
        const char* tmpname = "tmp_tree.txt";
        phylip_name = (char*)ckalloc(strlen(tmpname) + 1);
        strcpy(phylip_name, tmpname);
    }

    // Populate sequence lengths and names
    max_aln_length = 0;
    max_names = 0;

    for (uint seqI = 1; seqI <= nseqs; ++seqI) {
        const string& s = seq_table[seqI - 1];
        seqlen_array[seqI] = (sint)s.length();

        if (seqlen_array[seqI] > (sint)max_aln_length)
            max_aln_length = seqlen_array[seqI];

        // Assign ClustalW-safe name
        ostringstream ss;
        ss << "seq" << seqI;
        const string nm = ss.str();

        int namelen = std::min((int)nm.size(), MAXNAMES);
        strncpy(names[seqI], nm.c_str(), namelen);
        names[seqI][namelen] = '\0';

        strncpy(titles[seqI], nm.c_str(), namelen);
        titles[seqI][namelen] = '\0';

        if ((int)strlen(names[seqI]) > max_names)
            max_names = strlen(names[seqI]);

        // Allocate Clustal sequence storage
        alloc_seq(seqI, seqlen_array[seqI]);

        // Fill seq_array[seqI] with AA/DNA codes
        for (uint pos = 0; pos < s.length(); ++pos) {
            ...
            c = toupper(c);
            if (c == '-' || c == '.' || c == '~')
                seq_array[seqI][pos + 1] = gap_pos1;
            else {
                // convert to ClustalW alphabet index
                // (ClustalW uses amino_acid_codes[] mapping)
                sint idx = -1;
                for (sint i = 0; i <= max_aa; ++i) {
                    if (amino_acid_codes[i] == c) {
                        idx = i;
                        break;
                    }
                }
                if (idx < 0)
                    idx = gap_pos1;
                seq_array[seqI][pos + 1] = idx;
            }
        }
    }

    // Run pairwise and progressive alignment
    usemenu = FALSE;
    interactive = FALSE;
    output_clustal = TRUE;
    output_phylip = FALSE;
    ...
    // Perform pairwise alignment (sets distance matrix if needed)
    if (!pairalign(1, nseqs)) {
        if (phylip_name) ckfree(phylip_name);
        return false;
    }

    // If no external guide tree, build one internally
    if (phylip_name) {
        FILE* tmp = open_explicit_file(phylip_name);
        if (!tmp) {
            ckfree(phylip_name);
            return false;
        }
        guide_tree(tmp, 1, nseqs);
        fclose(tmp);
    }

    // Perform the progressive alignment
    if (!prfalign(1, nseqs)) {
        if (phylip_name) ckfree(phylip_name);
        return false;
    }

    // If a temporary guide tree was created, delete it
    if (phylip_name) {
        remove(phylip_name);
        ckfree(phylip_name);
    }

    // At this point seq_array contains the final alignment in ClustalW format.
    return true;
}
