...
void RefineW(const MSA &msaIn, MSA &msaOut)
{
    ...
    {
        if (!g_bQuiet.get())
            fprintf(stderr, "Window %u of %u    \r", uWindowIndex, uWindowCount);
        ...
        if (uWindowIndex == g_uSaveWindow.get())
        {
            ...
            char fn[256];
            snprintf(fn, sizeof(fn), "win%u_inaln.tmp", uWindowIndex);
            TextFile fIn(fn, true);
            msaInTmp.ToFile(fIn);

            snprintf(fn, sizeof(fn), "win%u_inseqs.tmp", uWindowIndex);
            TextFile fv(fn, true);
            v.ToFile(fv);

            snprintf(fn, sizeof(fn), "win%u_outaln.tmp", uWindowIndex);
